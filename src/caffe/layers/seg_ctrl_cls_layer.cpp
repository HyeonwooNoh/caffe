...
SegCtrlClsLayer<Dtype>::~SegCtrlClsLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void SegCtrlClsLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK(transform_param.has_mean_file() == false) << 
         "SegCtrlClsLayer does not support mean file";
  ...
  this->prefetch_cls_data_.Reshape(batch_size, 1, 1, 1);
  ...
  LOG(INFO) << "output ctrl data size: " << top[2]->num() << ","
	    ...
	    << top[2]->width();
  // cls data
  LOG(INFO) << "output cls data size: " << top[3]->num() << ","
	    << top[3]->channels() << "," << top[3]->height() << ","
	    << top[3]->width();
}

template <typename Dtype>
void SegCtrlClsLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void SegCtrlClsLayer<Dtype>::InternalThreadEntry() {
  ...
  Dtype* top_cls_data  = this->prefetch_cls_data_.mutable_cpu_data(); 
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int cls_label = lines_[lines_id_].cls_label;
    top_cls_data[item_id] = cls_label;
...
INSTANTIATE_CLASS(SegCtrlClsLayer);
REGISTER_LAYER_CLASS(SEG_CTRL_CLS, SegCtrlClsLayer);
}  // namespace caffe
