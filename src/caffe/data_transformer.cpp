...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgSegCtrl(const std::vector<cv::Mat>& cv_img_seg,
  Blob<Dtype>* transformed_data_blob, Blob<Dtype>* transformed_label_blob, 
  Blob<Dtype>* transformed_ctrl_blob, const int ignore_label) {
  CHECK(cv_img_seg.size() == 3) << "Input must contain image and seg.";
  ...
  const int ctrl_channels = cv_img_seg[2].channels();
  int ctrl_height   = cv_img_seg[2].rows;
  int ctrl_width    = cv_img_seg[2].cols;
  ...
  const int ctrlblob_channels = transformed_ctrl_blob->channels();
  const int ctrlblob_height   = transformed_ctrl_blob->height();
  const int ctrlblob_width    = transformed_ctrl_blob->width();
  ...
  CHECK_EQ(ctrl_channels, 1);
  CHECK_EQ(ctrl_height, seg_height);
  CHECK_EQ(ctrl_width, seg_width);

  CHECK_EQ(label_channels, 1);
  CHECK_EQ(data_height, label_height);
  CHECK_EQ(data_width, label_width);

  CHECK_EQ(ctrlblob_channels, 1);
  CHECK_EQ(data_height, ctrlblob_height);
  CHECK_EQ(data_width, ctrlblob_width);
  ...
  CHECK(cv_img_seg[2].depth() == CV_8U) << "Ctrl data type must be unsigned byte";
  ...
  cv::Mat cv_cropped_ctrl = cv_img_seg[2];
  ...
  if (pad_height > 0 || pad_width > 0) {
    ...
    cv::copyMakeBorder(cv_cropped_ctrl, cv_cropped_ctrl, 0, pad_height,
          0, pad_width, cv::BORDER_CONSTANT,
          cv::Scalar(0));
    ...
    ctrl_height   = cv_cropped_ctrl.rows;
    ctrl_width    = cv_cropped_ctrl.cols;
  }
  // crop img/seg
  if (crop_size) {
    ...
    cv_cropped_ctrl = cv_cropped_ctrl(roi);
  }

  CHECK(cv_cropped_img.data);
  CHECK(cv_cropped_seg.data);
  CHECK(cv_cropped_ctrl.data);
  ...
  for (int h = 0; h < data_height; ++h) {
    ...
    for (int w = 0; w < data_width; ++w) {
      ...
      pixel = static_cast<Dtype>(ctrl_ptr[ctrl_index++]);
      transformed_ctrl[top_index] = pixel;
    }
